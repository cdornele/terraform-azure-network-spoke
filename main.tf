...
resource "azurerm_virtual_network_dns_servers" "this" {
  count              = try(var.settings.dns_servers_enabled, false) ? 1 : 0
...
module "subnets" {
    ...
    for_each                = try(var.settings.subnet_settings.subnets, {})
    settings                = each.value
...
module "nsg" {
  ...
  resource_group                    = var.resource_group
  location                          = var.location
  subnets                           = try(var.settings.subnet_settings.subnets, {}) 
  settings                          = try(var.settings.network_security_group_settings, {})
  tags                              = merge(try(var.settings.tags, {}), try(var.settings.network_security_group_settings.tags, {}))
}
...
module "rts" {
  ...
  resource_group         = var.resource_group
  subnets                = try(var.settings.subnet_settings.subnets, {})
  settings               = try(var.settings.route_tables_settings, {})
  tags                   = merge(try(var.settings.tags, {}), try(var.settings.route_tables_settings.tags, {}))
}

resource "azurerm_subnet_route_table_association" "subnet_rts_association" {
  for_each              = {
                            for key, value in try(var.settings.subnet_settings.subnets, {}) : key => value
                            if try(var.settings.route_tables_settings[value.rts_key].is_enabled, false) && try(value.rts_key, null) != null
                          }
  subnet_id             = module.subnets[each.key].subnet_id
